-- 0022_aliases_diamond_apple_cider_parsley.sql
--
-- - Brand-named salt: Diamond Crystal kosher salt (and Morton's, while
--   we're at it) collapse into the existing salt bucket.
-- - apple cider vinegar -> champagne vinegar. User treats them as
--   interchangeable for the pantry; recipes commonly call for either.
-- - "parsley leaves and tender stems" (a common NYT-style recipe phrase)
--   and the more general "parsley leaves" / "fresh parsley" all collapse
--   to "parsley". Note: plural strip runs before alias lookup, so the
--   alias keys are the post-strip forms ("parsley leave", "parsley
--   leaves and tender stem"). Looks odd but is correct.
...
begin
  ...
  if s in (
    ...
    'grated parmigiano', 'pecorino or parmesan', 'parmesan or pecorino'
  ) then
