-- 0030_recipe_coverage_missing_names.sql
--
-- Surface the distinct canonical names of unmatched ingredients per
-- recipe so the home-page coverage pill can show "yogurt" instead of
-- "1 missing" when there's only one gap. For 0 or 2+ missing the pill
-- stays text-based ("ready" / "N missing") — the array is still returned
-- but the UI only reads it in the 1-missing case.
...
as $$
  ...
  select
    ...
    array_remove(
      array_agg(distinct c.norm_name) filter (where p.name is null and c.id is not null),
      null
    ) as missing_names
