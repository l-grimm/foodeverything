-- 0026_aliases_breadcrumb_veg_stock_sparkling_water.sql
--
-- - "bread crumb" -> "breadcrumb" (covers "bread crumbs" / "breadcrumbs"
--   via plural strip; spacing normalizes either side).
-- - vegetable stock joins the existing chicken-stock bucket. Mirrors the
--   chicken-bouillon alias from 0016. User treats stock types as
--   interchangeable for matching purposes.
-- - sparkling water -> water. Water is already in recipe_coverage's
--   assumed-staple array, so this folds sparkling water into the
--   "always present" treatment automatically.
...
begin
  ...
  if s in ('chicken bouillon', 'vegetable stock') then
